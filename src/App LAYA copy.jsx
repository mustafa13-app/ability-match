...
function getUpcomingClasses() {
  return buildUpcoming(schedule, false);
}
...
function getReminderStorageKey(reminderType, classItem) {
  return `laya-reminder-${reminderType}-${classItem.title}-${classItem.date.toISOString()}`;
}

export default function App() {
  const [upcoming, setUpcoming] = useState(getUpcomingClasses());
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setNow(new Date());
      setUpcoming(getUpcomingClasses());
    }, 1000);
  ...
  const comingNext = useMemo(() => {
    ...
    const optionalUpcoming = buildUpcoming(optionalClasses, true);
  ...
  async function enableReminders() {
    ...
    if (permission === "granted") {
      ...
      setActiveReminder({
        ...
        message: "Laya will get a heads-up before class starts.",
      });
      setHighlight(true);
  ...
  function showReminder(classItem, reminderType) {
    ...
    const body =
      reminderType === "five"
        ? `Get ready, Laya — ${classItem.title} starts at ${formatClassTime(classItem.time)}.`
  ...
  useEffect(() => {
    ...
    const fiveMinuteKey = getReminderStorageKey("five", next);
    const startKey = getReminderStorageKey("start", next);
        ...
        >
          ...
          >
            <div style={{ textAlign: "left", color: "white" }}>
              <div style={{ fontSize: "20px", fontWeight: "700" }}>Hi Laya 🌟</div>
